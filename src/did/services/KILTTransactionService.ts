...
export class KILTTransactionService {
  ...
  public async submitSignedTransaction(
    ...
    try {
      ...
      const result: KILTTransactionResult = {
        ...
        fee: await this.estimateFeeFromExtrinsic(signedExtrinsic),
   ...
   * @private
   */
  private async estimateFeeFromExtrinsic(signedExtrinsic: any): Promise<{ amount: string; currency: string }> {
    try {
      // Try to get fee estimation from the signed extrinsic
      // This is a simplified version - can be enhanced with actual fee calculation
      return {
        amount: '0',
        currency: 'KILT',
      };
