...
import { KILT_NETWORKS, KILTNetwork } from '../config/kiltConfig';
import { KILTTransactionService } from '../did/services/KILTTransactionService';
import { 
  KILTTransactionResult, 
  KILTTransactionEvent,
  KILTError,
  KILTErrorType
} from '../did/types/KILTTypes';
...
export class KiltAdapter implements WalletAdapter {
  ...
  public async checkBalance(
    ...
    try {
      ...
      // Get free and reserved balances (using type assertion for compatibility)
      const accountData = accountInfo as any;
      const freeBalance = accountData.data.free.toBigInt();
      const reservedBalance = accountData.data.reserved.toBigInt();
  ...
  public async signTransaction(
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('User rejected')) {
        throw new UserRejectedError('signing');
      }
  ...
  private startTransactionMonitoring(): void {
    ...
    this.transactionMonitorInterval = setInterval(async () => {
      if (this.pendingTransactions.size > 0) {
        Array.from(this.pendingTransactions.entries()).forEach(async ([txHash, status]) => {
          try {
            if (this.transactionService) {
        ...
        Array.from(this.pendingTransactions.entries()).forEach(([txHash, status]) => {
