...
import { MoonbeamDIDService } from '../did/services/MoonbeamDIDService';
import { 
  MoonbeamTransactionResult, 
  MoonbeamError, 
  MoonbeamErrorType 
} from '../did/types/MoonbeamTypes';
...
export class BlockchainMonitor {
  ...
  public async monitorMoonbeamDIDTransaction(
    ...
    try {
      ...
      if (receipt) {
        ...
        transaction.gasUsed = BigInt(receipt.gasUsed);
        transaction.cost = BigInt(receipt.gasUsed) * (receipt.gasPrice || BigInt(0));
        transaction.confirmations = 1;
      }
  ...
  private async handleTransactionFailure(
    ...
    if (this.isRetryableError(error) && transaction.retryCount < transaction.maxRetries) {
      ...
      // Retry based on blockchain type - only Moonbeam supported
  ...
  /**
   * Perform health check for Moonbeam DID
   */
